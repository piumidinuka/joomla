...
class DesignerPage
{
    ...
    {
        //remove themler plugin files
        $doc = JFactory::getDocument();

        $cmsJquery = JURI::root(true) . '/media/jui/js/jquery.min.js';
        if (isset($doc->_scripts[$cmsJquery]))
            unset($doc->_scripts[$cmsJquery]);
        $cmsBootstrap = JURI::root(true) . '/media/jui/js/bootstrap.min.js';
        if (isset($doc->_scripts[$cmsBootstrap]))
            unset($doc->_scripts[$cmsBootstrap]);

        $pluginLibFolder = JURI::root(true) . '/plugins/content/themlercontent/lib';
        if (isset($doc->_scripts[$pluginLibFolder . '/jquery.js']))
            unset($doc->_scripts[$pluginLibFolder . '/jquery.js']);
        if (isset($doc->_scripts[$pluginLibFolder . '/beforejq.js']))
                    unset($doc->_scripts[$pluginLibFolder . '/beforejq.js']);
        if (isset($doc->_scripts[$pluginLibFolder . '/afterjq.js']))
                    unset($doc->_scripts[$pluginLibFolder . '/afterjq.js']);
        if (isset($doc->_scripts[$pluginLibFolder . '/bootstrap.min.js']))
            unset($doc->_scripts[$pluginLibFolder . '/bootstrap.min.js']);
        if (isset($doc->_styleSheets[$pluginLibFolder . '/bootstrap.css']))
            unset($doc->_styleSheets[$pluginLibFolder . '/bootstrap.css']);
        $GLOBALS['themlerThemePluginFilesDeleted'] = '1';
    ...
    public function renderTemplate($themeDir)
    {
        ...
        $template = funcContentRoutesCorrector($template);
        return $template;
    }
    ...
    public function componentWrapper($template)
    {
        ...
        // replace links for component
        $content = str_replace('tmpl=component&', 'tmpl=component&componentStyle=' . $template . '&', $content);
